...
export class FalAIService {
  ...
  async processImage(
    ...
    try {
      ...
      if (operation !== 'topaz-upscale') {
        ...
        console.log(`🎯 Processing ${operation} (upscale - no prompt required)`);
      }
  ...
  private async topazUpscale(imageInput: string, parameters: Record<string, any>) {
    return await fal.subscribe('fal-ai/topaz/upscale/image', {
      ...
      onQueueUpdate: (update) => {
        if (update.status === "IN_PROGRESS") {
          console.log('🔍 Topaz Upscale:', update.logs?.map(log => log.message).join(', '));
        }
        if (update.status === "COMPLETED") {
          console.log('✅ Topaz Upscale completed');
        }
        if (update.status === "FAILED") {
          console.error('❌ Topaz Upscale failed:', update.error);
        }
      }
    });
  }
  ...
  async processImageWithUrl(
    ...
    try {
      console.log(`🎯 Processing ${operation} with image URL: ${imageUrl}`);
      ...
      console.log(`✅ Fal.ai response received for ${operation}:`, {
        hasData: !!result.data,
        requestId: result.requestId
      });
      ...
      console.log(`📥 Downloading processed image from: ${processedImageUrl}`);
